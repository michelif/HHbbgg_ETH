...
for idx_nn,nn in enumerate(input_trainings): 
    ...
    if idx_nn==0 :  
        for i,column in enumerate(data_frames[0].columns.values.tolist()):
            dictVar[column] = i
    mean = np.mean( np.array( [ data_frames[i].as_matrix() for i in range(0,xval)] ), axis=0 )
    mean2 = np.mean( np.array( [ data_frames[i].as_matrix() for i in range(0,xval)] )**2, axis=0 )
...
for idx,met in enumerate(input_metrics):
    for idx_nn,nn in enumerate(input_trainings): 
        ...
        std = all_stds[idx_nn]
        plt.plot(mean[:,dictVar['epoch']],mean[:,dictVar['%s'%met]],linestyle='-',label='%s Training'%(nn.split('HybridLoss')[1]),color=colors[idx_nn])
        plt.scatter(mean[:,dictVar['epoch']],mean[:,dictVar['val_%s'%met]],label='%s Validation'%(nn.split('HybridLoss')[1]),color=colors[idx_nn])
        plt.fill_between(mean[:,dictVar['epoch']],mean[:,dictVar['val_%s'%met]]-std[:,dictVar['val_%s'%met]],mean[:,dictVar['val_%s'%met]]+std[:,dictVar['val_%s'%met]],alpha=0.2,label='%s Validation std'%(nn.split('HybridLoss')[1]),color=colors[idx_nn])
    plt.xlabel('# epochs')
    plt.ylabel('%s'%met)
    plt.legend()
