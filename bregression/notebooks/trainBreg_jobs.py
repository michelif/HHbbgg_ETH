...
X_train_target = preprocessing.get_training_sample(pd.DataFrame(X_target),0.1)
X_train_features = preprocessing.get_training_sample(pd.DataFrame(X_features),0.1)
X_train_weights = preprocessing.get_training_sample(pd.DataFrame(X_weights),0.1)
...
#       learning_rate=0.2, max_delta_step=0, max_depth=10,
 #      min_child_weight=0.02, missing=None, n_estimators=700, nthread=2,
  #     objective='reg:linear', reg_alpha=0, reg_lambda=1.0,
   #    scale_pos_weight=1, seed=0, silent=True, subsample=1.0)   #best parameters found for non pt_reweighting
##########################################
####
...
clf = optimization.optimize_parameters_randomizedCV_reg(xgb_model,X_train_features,X_train_target,X_test_features,X_test_target,param_grid,my_scorer,1,2,50,True,X_train_weights)#nIter=20,cvOpt=5,nJobs=20
