...
#ifdef macro_breg_cxx
macro_breg::macro_breg(TString filename) : fChain(0) 
{
...
void macro_breg::AddFile(TString filename)
{
	file_name = filename;
...
macro_breg::~macro_breg()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t macro_breg::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t macro_breg::LoadTree(Long64_t entry)
{
...
void macro_breg::Init(TString filename)
{
...
void macro_breg::Show(Long64_t entry)
{
...
Int_t macro_breg::Cut(Long64_t entry)
{
